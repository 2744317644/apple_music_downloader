...
func (a *Station) GetResp(mutoken, token, l string) error {
	...
	if err != nil {
		return fmt.Errorf("error getting station response: %w", err)
	}
	a.Resp = *resp
	//简化高频调用名称
	...
	if err != nil {
		return fmt.Errorf("error getting station tracks response: %w", err)
	}
	//fmt.Println("Getting album response")
	//从resp中的Tracks数据中提取trackData信息到新的Track结构体中
	for i, trackData := range tracksResp.Data {
		albumResp, err := ampapi.GetAlbumRespByHref(trackData.Href, a.Language, token, mutoken)
		if err != nil {
