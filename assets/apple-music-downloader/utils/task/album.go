...
func (a *Album) GetResp(token, l, mediaUserToken string) error {
	var err error
	a.Language = l
	resp, err := ampapi.GetAlbumResp(a.Storefront, a.ID, a.Language, token, mediaUserToken)
	if err != nil {
		return fmt.Errorf("error getting album response: %w", err)
	}
	a.Resp = *resp
	//简化高频调用名称
